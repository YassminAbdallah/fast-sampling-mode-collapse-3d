...
def main():
    ...
    parser.add_argument("--pca-dim", type=int, default=128,
                        help="PCA dim for k-NN feature space. Set 0 to disable (use raw 4096-D). "
                             "Default 128 follows Kynkäänniemi et al. (2019) Inception-2048→128 "
                             "convention and matches the dimensionality reduction used for "
                             "Latent FID elsewhere in this paper. v7.2 peer-review fix.")
    ...
    real_features_raw = extract_features(vols, enc, dev)
    print(f"  Real feature shape (raw): {real_features_raw.shape}")

    # Fit PCA on real features once, then transform both real and generated
    # features into the same lower-dimensional space. Following Kynkäänniemi
    # et al. (2019), k-NN manifold estimation is unreliable in very high
    # dimensions; we project to 128-D (matching the Latent FID protocol).
    if args.pca_dim and args.pca_dim < real_features_raw.shape[1]:
        n_pca = min(args.pca_dim, real_features_raw.shape[0])
        pca = PCA(n_components=n_pca, random_state=0)
        pca.fit(real_features_raw)
        real_features = pca.transform(real_features_raw)
        print(f"  PCA: {real_features_raw.shape[1]} -> {real_features.shape[1]} "
              f"(explained variance: {pca.explained_variance_ratio_.sum():.3f})")
    else:
        pca = None
        real_features = real_features_raw
        print(f"  PCA: disabled, using raw {real_features.shape[1]}-D features")

    out_dir = data_dir / "precision_recall_pca128" if args.pca_dim else data_dir / "precision_recall"
    ...
    for method, step_counts in METHODS_TO_TEST:
        ...
        for steps in step_counts:
            ...
            for seed in args.seeds:
                ...
                gen_features_raw = extract_features(gen_vols, enc, dev)
                # Apply the same PCA fitted on the real-data distribution
                if pca is not None:
                    gen_features = pca.transform(gen_features_raw)
                else:
                    gen_features = gen_features_raw
    ...
    save_data = {
        "metadata": {
            ...
            "feature_dim_raw": real_features_raw.shape[1],
            "feature_dim_used": real_features.shape[1],
            "pca_dim": args.pca_dim if pca is not None else 0,
            "pca_explained_variance_ratio_sum": float(pca.explained_variance_ratio_.sum())
                if pca is not None else None,
            "n_real": len(real_features),
            "note": (
                "v7.2: P/R and D/C are computed in PCA-reduced feature space "
                "(default 128-D from raw 4096-D VQ-GAN encoder output). PCA is "
                "fitted on the real-data distribution once and applied to all "
                "generated samples. This matches the Kynkäänniemi et al. (2019) "
                "Inception-2048→128 convention and the Latent FID protocol used "
                "elsewhere in this paper. The pre-v7.2 results in "
                "results/.../precision_recall/ used raw 4096-D features and "
                "should be considered superseded for any future use; new "
                "outputs are written to .../precision_recall_pca128/ to "
