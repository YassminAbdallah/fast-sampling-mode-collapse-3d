...
@torch.no_grad()
def generate_samples(unet, dec, method, n, dev, steps, ch=8, batch_size=8, class_label=None):
...
def evaluate_method(unet, dec, method, vols, dev, step_counts, n_samples=64, ch=8, class_label=None):
...
def main():
    ...
    parser.add_argument("--data-path", type=str, required=True,
                        help="Path to preprocessed data .pt file (standard or conditional)")
    ...
    parser.add_argument("--class-label", type=int, default=None,
                        help="Class label for conditional generation (default: generate mixed)")
    ...
    if args.conditional:
        print(f"Conditional mode: num_classes={args.num_classes}")
        num_classes = args.num_classes
        cond_data = torch.load(args.data_path, weights_only=False)
        vols = cond_data["volumes"]
        labels = cond_data["labels"]
        # Use training split for real data comparison
        train_idx = cond_data["split_info"]["train_idx"]
        vols = vols[train_idx]
        labels = labels[train_idx]
        print(f"Loaded {len(vols)} training volumes (conditional)")
        print(f"  Label type: {cond_data.get('label_type', '?')}")
        print(f"  Class distribution: {[(labels == c).sum().item() for c in range(num_classes)]}")
    ...
    for method, step_counts in STEP_COUNTS.items():
        ...
        results = evaluate_method(unet, dec, method, vols, dev, step_counts,
                                   args.n_samples, class_label=class_label)
