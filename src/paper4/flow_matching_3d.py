...
class BrainDS(Dataset):
    def __init__(self, vols, labels=None):
        self.v = vols
        self.labels = labels
    def __len__(self):
        return len(self.v)
    def __getitem__(self, i):
        item = {"volume": self.v[i]}
...
class DenoisingUNet3D(nn.Module):
    ...
    For conditional: called as unet(x, t, class_label=c) or unet(x, t, d=d, class_label=c).
...
class LatentGenerative3D(nn.Module):
    def __init__(self, cfg, method="fm"):
        ...
        self.num_classes = cfg.get("num_classes", 0)
        ...
        self.unet = DenoisingUNet3D(ch, num_classes=self.num_classes)
    ...
    # ---- DDPM loss ----
    def ddpm_loss(self, x, class_label=None):
        ...
        return F.mse_loss(self.unet(self.sch.add_noise(zq, noise, t), t.float(), class_label=class_label), noise)

    # ---- Standard FM loss ----
    def fm_loss(self, x, class_label=None):
        with torch.no_grad(): zq, _, _ = self.vq(self.enc(x))
        z0 = torch.randn_like(zq); t = torch.rand(zq.shape[0], device=x.device)
        te = t[:, None, None, None, None]
        xt = (1 - te) * z0 + te * zq
        return F.mse_loss(self.unet(xt, t, class_label=class_label), zq - z0)

    # ---- Shortcut FM losses ----
    def shortcut_fm_loss(self, x, class_label=None):
        ...
        return F.mse_loss(self.unet(xt, t, d=d_zero, class_label=class_label), target)

    def shortcut_sc_loss(self, x, d_max=1.0, class_label=None):
        ...
        v_big = self.unet(xt, t, d=2*d, class_label=class_label)
        x_big_step = xt + 2 * de * v_big

        # TWO small steps: [stop-gradient target]
        with torch.no_grad():
            v1 = self.unet(xt, t, d=d, class_label=class_label)
            x_mid = xt + de * v1              # first d-step: t -> t+d
            v2 = self.unet(x_mid, t + d, d=d, class_label=class_label)
            x_two_steps = x_mid + de * v2     # second d-step: t+d -> t+2d

        return F.mse_loss(x_big_step, x_two_steps.detach())

    # ---- Reflow loss ----
    def reflow_loss(self, x, teacher_unet, class_label=None):
        with torch.no_grad():
            zq, _, _ = self.vq(self.enc(x))
            z0 = torch.randn_like(zq); z = z0.clone()
            for i in range(10):
                t_t = torch.full((z.shape[0],), i / 10.0, device=z.device)
                z = z + teacher_unet(z, t_t, class_label=class_label) * 0.1
            z1 = z
        t = torch.rand(z0.shape[0], device=x.device)
        te = t[:, None, None, None, None]
        xt = (1 - te) * z0 + te * z1
        return F.mse_loss(self.unet(xt, t, class_label=class_label), z1 - z0)

    # ---- Consistency loss ----
    def consistency_loss(self, x, teacher_unet, n_steps, ema_unet=None, class_label=None):
        ...
        v_student = self.unet(x_tn, t_n, class_label=class_label)
        ...
        with torch.no_grad():
            v_target = target_net(x_tn1, t_n1, class_label=class_label)
            f_target = x_tn1 + (1 - te_n1) * v_target
        return F.mse_loss(f_student, f_target)

    # ---- Loss dispatch ----
    def phase2_loss(self, x, class_label=None, **kw):
        if self.method == "ddpm": return self.ddpm_loss(x, class_label=class_label)
        elif self.method == "fm": return self.fm_loss(x, class_label=class_label)
        elif self.method == "rectified": return self.reflow_loss(x, kw["teacher_unet"], class_label=class_label)
        elif self.method == "consistency": return self.consistency_loss(
            x, kw["teacher_unet"], kw["n_steps"], kw.get("ema_unet"), class_label=class_label)
            ...
            if torch.rand(1).item() < sc_ratio:
                return self.shortcut_sc_loss(x, d_max=d_max, class_label=class_label)
            else:
                return self.shortcut_fm_loss(x, class_label=class_label)

    # ---- DDPM sampling ----
    @torch.no_grad()
    def ddpm_sample(self, n, dev, steps, class_label=None):
        ch = self.cfg["latent_channels"]; z = torch.randn(n, ch, 8, 8, 8, device=dev)
        cl = self._prep_class_label(class_label, n, dev)
        ss = max(self.sch.T // steps, 1); ts = list(range(self.sch.T - 1, -1, -ss))
        for tv in ts:
            t = torch.full((n,), tv, device=dev, dtype=torch.float32)
            np_ = self.unet(z, t, class_label=cl)
    ...
    @torch.no_grad()
    def ode_sample(self, n, dev, steps, class_label=None):
        ch = self.cfg["latent_channels"]; z = torch.randn(n, ch, 8, 8, 8, device=dev)
        cl = self._prep_class_label(class_label, n, dev)
        dt = 1.0 / steps
        for i in range(steps):
            t = torch.full((n,), i * dt, device=dev); z = z + self.unet(z, t, class_label=cl) * dt
        return self.decode(z)

    # ---- Consistency sampling ----
    @torch.no_grad()
    def consistency_sample(self, n, dev, steps, class_label=None):
        ...
        if steps == 1:
            ...
            v = self.unet(z, t, class_label=cl)
    ...
    # ---- Shortcut sampling ----
    @torch.no_grad()
    def shortcut_sample(self, n, dev, steps, class_label=None):
        ...
        for i in range(steps):
            ...
            v = self.unet(z, t, d=d, class_label=cl)
            z = z + d_val * v
        return self.decode(z)

    # ---- Helper: prepare class labels for sampling ----
    def _prep_class_label(self, class_label, n, dev):
        """Convert class_label to proper tensor for sampling methods."""
        if class_label is None or self.num_classes == 0:
            return None
        if isinstance(class_label, int):
            return torch.full((n,), class_label, device=dev, dtype=torch.long)
        if isinstance(class_label, torch.Tensor):
            return class_label.to(dev)
        return torch.tensor([class_label] * n, device=dev, dtype=torch.long)

    # ---- Sampling dispatch ----
    def generate(self, n=1, dev="cpu", steps=50, class_label=None):
        if self.method == "ddpm": return self.ddpm_sample(n, dev, steps, class_label=class_label)
        elif self.method == "consistency": return self.consistency_sample(n, dev, steps, class_label=class_label)
        elif self.method == "shortcut": return self.shortcut_sample(n, dev, steps, class_label=class_label)
        else: return self.ode_sample(n, dev, steps, class_label=class_label)
...
def train_p2_standard(model, dl, cfg, dev, rdir):
    ...
    for e in range(1, ep + 1):
        ...
        for batch in dl:
            ...
            if cl is not None: cl = cl.to(dev)
            l = model.phase2_loss(batch["volume"].to(dev), class_label=cl)
...
def train_reflow(model, dl, cfg, dev, rdir, teacher_state):
    ...
    for e in range(1, ep + 1):
        ...
        for batch in dl:
            cl = batch.get("label", None)
            if cl is not None: cl = cl.to(dev)
            l = model.phase2_loss(batch["volume"].to(dev), teacher_unet=teacher, class_label=cl)
            opt.zero_grad(); l.backward()
            torch.nn.utils.clip_grad_norm_(params, 1.0); opt.step()
            el += l.item(); n += 1
        avg = el/n; losses.append(avg); sch.step()
    ...
    for e in range(1, ep + 1):
        ...
        for batch in dl:
            ...
            if torch.rand(1).item() < sc_ratio and d_max > 1/64:
                l = model.shortcut_sc_loss(x, d_max=d_max, class_label=cl)
                l_sc += l.item(); n_sc += 1
            else:
                l = model.shortcut_fm_loss(x, class_label=cl)
                l_fm += l.item(); n_fm += 1
            opt.zero_grad(); l.backward()
            torch.nn.utils.clip_grad_norm_(params, 1.0); opt.step()
            el += l.item()
...
def run_shared_phase1(cfg, vols, dev, rdir, labels=None):
    ds = BrainDS(vols, labels=labels)
...
def run_method(cfg, vols, p1_state, dev, rdir, method, fm_unet_state=None, labels=None):
    ds = BrainDS(vols, labels=labels)
    dl = DataLoader(ds, batch_size=cfg["batch_size"], shuffle=True, num_workers=0, drop_last=True)
...
def run_comparison(cfg, vols, dev, base_dir, fraction=1.0, methods=None, labels=None):
    if methods is None: methods = ALL_METHODS
    n = max(2, int(len(vols) * fraction))
    fvols = vols[:n]
    flabels = labels[:n] if labels is not None else None
    fdir = base_dir / f"{int(fraction*100)}pct_{n}vol"
    fdir.mkdir(parents=True, exist_ok=True)
    print(f"\n{'='*60}\n  BENCHMARK: {cfg.get('dataset','ixi').upper()} | {n} vol ({fraction*100:.0f}%) | {', '.join(m.upper() for m in methods)}\n{'='*60}")

    p1_state, _ = run_shared_phase1(cfg, fvols, dev, fdir, labels=flabels)
    all_results = {}; all_losses = {}; fm_unet = None
    ordered = [m for m in ["ddpm", "fm", "rectified", "consistency", "shortcut"] if m in methods]

    for method in ordered:
        res, losses, unet = run_method(cfg, fvols, p1_state, dev, fdir, method, fm_unet, labels=flabels)
...
def run_full_paper(cfg, vols, dev, base_dir, methods=None, labels=None):
    fracs = [0.10, 0.25, 0.50, 1.0]; all_res = {}
    for f in fracs:
        all_res[f"{int(f*100)}%"] = run_comparison(cfg, vols, dev, base_dir, f, methods, labels=labels)
...
def main():
    ...
    # Ablation controls for shortcut
    parser.add_argument('--sc-ratio', type=float, default=None,
                        help='Shortcut SC loss ratio (default 0.25). Use 0.0 for FM-only ablation.')
    parser.add_argument('--no-curriculum', action='store_true',
                        help='Disable d_max curriculum (use d_max=1.0 from start)')
    parser.add_argument('--ablation-name', type=str, default=None,
                        help='Custom name for ablation run directory (e.g. "sc0.10", "no_curriculum")')
    # Conditional generation (Paper 4)
    parser.add_argument('--conditional', action='store_true',
                        help='Enable class-conditional generation')
    parser.add_argument('--num-classes', type=int, default=2,
                        help='Number of classes for conditional generation (default: 2)')
    parser.add_argument('--cond-data-path', type=str, default='data/brats_conditional_64.pt',
                        help='Path to conditional dataset (from prepare_conditional_brats.py)')
    ...
    # Ablation overrides
    if args.sc_ratio is not None:
        cfg["shortcut_sc_ratio"] = args.sc_ratio
    if args.no_curriculum:
        cfg["shortcut_d_max_schedule"] = False
    ...
    # ---- Load data (conditional or standard) ----
    labels = None
    if args.conditional and os.path.exists(args.cond_data_path):
        print(f"Loading conditional dataset: {args.cond_data_path}")
        cond_data = torch.load(args.cond_data_path, weights_only=False)
        ...
        cfg["max_volumes"] = len(vols)
        print(f"Conditional data: {len(vols)} training volumes, "
              f"{cfg['num_classes']} classes ({cond_data.get('label_names', ['?','?'])})")
        print(f"  Class distribution: {[(labels == c).sum().item() for c in range(cfg['num_classes'])]}")
    elif args.conditional:
        print(f"WARNING: --conditional specified but {args.cond_data_path} not found.")
        print("Run prepare_conditional_brats.py first. Falling back to unconditional.")
        cfg["num_classes"] = 0; cfg["conditional"] = False
        vols = load_data(cfg)
    else:
        vols = load_data(cfg)
    ...
    if args.resume_dir:
        ...
        # Ablation naming: save to distinct directory
        if args.ablation_name:
            method_dirname = f"{method}_{args.ablation_name}"
        elif method == "shortcut" and (args.sc_ratio is not None or args.no_curriculum):
            # Auto-name based on ablation settings
            parts = [method]
            if args.sc_ratio is not None:
                parts.append(f"sc{args.sc_ratio:.2f}")
            if args.no_curriculum:
                parts.append("nocurr")
            method_dirname = "_".join(parts)
        else:
            method_dirname = method
        
        mdir = data_dir / method_dirname; mdir.mkdir(parents=True, exist_ok=True)
        print(f"  Output dir: {mdir}")
        model = LatentGenerative3D(cfg, method=method).to(dev)
        model.enc.load_state_dict(p1_state["enc"])
        model.dec.load_state_dict(p1_state["dec"])
        model.vq.load_state_dict(p1_state["vq"])
    ...
    if args.full_paper: run_full_paper(cfg, vols, dev, base_dir, methods, labels=labels)
    else: run_comparison(cfg, vols, dev, base_dir, args.fraction, methods, labels=labels)
