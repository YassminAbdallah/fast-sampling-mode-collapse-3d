#!/usr/bin/env python3
"""
A1 evaluation: sample from each retrained model and compare to the
original at NFE = 4 (Consistency) / 50 (Shortcut) using the paper's
own 5-sampling-seed protocol.

Produces the payoff table for the A1 rebuttal:

    (method, train_seed) -> {SSIM, PSNR, diversity, %real} mean ± std
    over 5 sampling seeds.

Then the paper's §5.2 gains a paragraph saying "across 3 training seeds
of CD, cross-train-seed std on diversity-%-of-real is X, well below the
14x gap to Shortcut FM under matched training-seed variation."
"""
...
REPLICA   = REPO_ROOT / "results/paper4/train_seed_replication"
DATA_PATH = REPO_ROOT / "data/brats_conditional_64.pt"

# Which (method, label, ckpt, train_seed_note) rows to evaluate.
# The originals count as one training seed each ("original" = whatever seed
# was implicit in the initial main-benchmark run).
DEFAULT_MODELS = [
...
@torch.no_grad()
def generate(unet, dec, method, n, dev, steps, ch=8, batch_size=8, class_label=1):
    """Same shape as the paper's evaluate_5seed generate_samples."""
    out = []
    for start in range(0, n, batch_size):
        bs = min(batch_size, n - start)
        z = sample_latent(unet, method, bs, dev, steps, ch, class_label=class_label)
        out.append(dec(z).clamp(0, 1).cpu())
    return torch.cat(out, 0)


def eval_one_model(method, ckpt_path, real_np, real_sample, real_div, dev, steps_list,
                    n_samples=64, num_classes=2, class_label=1):
    print(f"\n  Loading {method}: {ckpt_path}")
    if not ckpt_path.exists():
        print(f"    MISSING (skip)")
        return None

    # Robust loading (nested 'unet' key sometimes)
    raw = torch.load(ckpt_path, map_location=dev, weights_only=True)
    if isinstance(raw, dict) and "unet" in raw and isinstance(raw["unet"], dict):
        # dedicated model save
        # Fall through to load_unet which handles either flat or 'unet.'-prefixed
        pass
    unet = load_unet(ckpt_path, dev, num_classes=num_classes)
    unet.eval()

    # Rebuild decoder from VQ-GAN (same for all)
...
def main():
    ...
    ap.add_argument("--output", default="results/paper4/train_seed_replication/a1_eval_summary.json")
    ...
    print(f"real_diversity (n={len(real_sample)}) = {real_div:.4f}")
    ...
    # ----- Cross-training-seed statistics per method -----
    print("\n" + "=" * 70)
    print("A1 headline: cross-training-seed variance per method")
    print("=" * 70)
    a1_summary = {}
    for method, per_label in all_results.items():
        for steps in STEPS_FOR[method]:
            step_key = str(steps)
            divs = [per_label[l][step_key]["diversity_mean"]
                    for l in per_label
                    if step_key in per_label[l]]
            ...
            if len(divs) >= 2:
                ...
                print(f"  {key}: mean diversity {cross_mean:.1f}% real  "
                      f"cross-train-seed std {cross_std:.1f} pp  "
                      f"(vs within-train-seed sampling std {mean_sampling_std_pct:.2f} pp)")
